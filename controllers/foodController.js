    ...
    try {
        ...
        if (expiringItems.length > 0 && process.env.GEMINI_API_KEY) {
            ...
            try {
                const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
                const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
                const prompt = `I have the following food items expiring in my fridge: ${itemsListText}. Suggest a creative meal I can cook using some or all of these items. Provide the name of the meal and a 1-2 sentence short description. Keep it brief. Return it formatted nicely.`;
